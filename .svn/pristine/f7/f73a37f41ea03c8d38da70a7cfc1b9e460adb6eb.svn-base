function doValidForm(ob) {
		...
		$(ob).find("input[require]:visible").blur(function(){
			...
			if (temp.trim() != '') {
				...
				$(this).next().addClass('ff4 ').text(field+' can not empty').fadeIn();		
				flag = false;
			}
		...
		$(ob).find("textarea[requireOrtext]:visible").blur(function(){
			...
			if (temp.trim() != '') {
				...
				$('#equipmentWarn').addClass('ff4').text(field+' can not empty').fadeIn();		
				flag = false;
			}
		...
		$(ob).find("input[requiredate]:visible").blur(function(){
		    ...
		    if (!temp||temp.trim()==''){
				...
				$(this).parent().next().addClass('ff4').text(field+' can not empty').show();		
				flag = false;
			}else if(!a.test(temp)){
				if($(this).attr('place')){
					$(this).attr('placeholder',$(this).attr('place'));
				}
					$(this).parent().next().addClass('ff4').text(field+' date format is incorrect').show();		
					flag = false;
			}else{
				$(this).addClass('success');
				flag = true;
		...
		$(ob).find("select[require]:visible").blur(function(){
			...
			if (temp.trim() != '') {
				...
				$(this).next().addClass('ff4').text(field+' is needed').fadeIn();		
				flag = false;
			}
			return false;
		...
		$(ob).find("select[requireOrtext]:visible").blur(function(){
			...
			if (temp.trim() != '') {
             ...
             $("#equipmentWarn").addClass('ff4').text(field+' is needed').fadeIn();		
				flag = false;
		...
		$(ob).find("input[invitationRequireNum ]:visible").blur(function(){
			...
			if (temp == '') {
				flag = false;
				$(this).parent().children().last().addClass('ff4-2').text(field+" can not empty").fadeIn();
				return false;

			}

			else if (!$.isNumeric(temp)) {
				$(this).parent().children().last().addClass('ff4-2').text(field+" must be a number").fadeIn();
				flag = false;
				return false;
			}
	   ...
	   $(ob).find("input[quicklyRequireNum ]:visible").blur(function(){
			...
			if (temp == '') {
				flag = false;
				$(this).parent().parent().children().last().addClass('ff4').text(field+" can not empty").fadeIn();
				return false;

			}

			else if (!$.isNumeric(temp)) {
				$(this).parent().parent().children().last().addClass('ff4').text(field+" must be a number").fadeIn();
				flag = false;
				return false;
			}
