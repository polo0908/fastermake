$(function() {
	...
	if (factory != null&& factory != '') {
		...
		$('#inviteFactory').html('<option value="'+factoryId+'">Selected a supplier</option>')
...
//	var deliveryDate = dateString2.substr(0,4)+'-'+appendZero(dateString2.substr(5,2))+'-'+appendZero(dateString2.substr(8,2);
			        ...
			        if($('#mainFactoryForeach > div').length>0){
                        ...
                        easyDialog.open({
                            container : {
                                ...
                                content :  'Invite quotation factory list is empty'
                            },
                            overlay : false,
                            autoClose : 1500
                        });
	...
	$("#saveButton").click(function() {
		...
		if($('#factoryList').is(':visible')){
			if($('#mainFactory').val()==''){
				$('#factoryListV').addClass('ff4').text('Invite quotation factory list is empty')
			}
	...
	//	$('.weui_mask_new').show();
		...
		$('.product_invitation').find('input[type=text],input[type=hidden],select,textarea').each(function() {
			    ...
			    if($(this).attr('name') == 'qualification' && $(this).val() == 'None'){
                        data[$(this).attr('name')] = '';
				}else{
	...
	'<label for="" class="col-sm-2 control-label"><span></span></label>'+
		...
		'<div class="col-sm-10 colsm10 colsm102 ff5div">'+
			'<input type="text" class="form-control" name="quantityList"'+
				'place="0" placeholder="0" field="Quantity" invitationRequireNum>'+
			'<select class="form-control" name="quantityUnit">'+
				'<option>pcs</option>'+
				'<option>ton</option>'+
			'</select> <span class="s0 deleteQuantityList" onclick="deleteQuantityList(this)" style="magin-left:600px">- Delete</span>  <span class="ff5" ></span></div>';
...
function checkDelivery(){
	if($('#quoteEndDate').val()!=''&&$('#deliveryDate').val()!=''){
		if($('#quoteEndDate').val()>$('#deliveryDate').val()){
			$('#deliveryDate').parent().next().addClass('ff4').text('Delivery date must be later than quotation deadline').show()
...
function checkDeadline(){
	if($('#quoteEndDate').val()!=''){		
		...
		if($('#quoteEndDate').val()<now){
			$('#quoteEndDate').parent().next().addClass('ff4').text('Deadline must be later than today').show()			
		}
	}
}	
