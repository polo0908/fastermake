$(function() {
	...
	$('#planInfo').blur(function() {
		...
		if (planInfo.trim() != '') {
			...
			$('#planInfo').next().addClass('ff4 s0').text("Content can not empty!").fadeIn();
	...
	$('#createNewQuote').click(function(){
		...
		if(factoryId){
			window.location = "/en/quickly_release.html?factoryId="+factoryId;
		}
			...
			.ajax({
				...
				success : function(result) {
					...
					if (result.state == 0) {
						...
						$('#productInfo2').text(productLibrary.productInfoEn);
						$('#planBuyCount').parent().next().find('span').text(
								'Minimum purchase ' + (productLibrary.minAqmountPurchased?productLibrary.minAqmountPurchased:'-')
										+ ("Piece"));
						// $('#planBuyPrice').parent().next().find('span').text(
						// 		'人民币/ 件（价格范围区间: ' + productLibrary.minquote
						// 				+ '）')
						$('#planBuyCountUnit').val("Piece")

						$('#quote').text(
								'￥' + productLibrary.minquote + '/'
										+ ("Piece"))
						$('#minAqmountPurchased').text((productLibrary.minAqmountPurchased?productLibrary.minAqmountPurchased+("Piece"):""));
						$('.f3')
