						...
						if (noteMessageList && noteMessageList.length > 0) {
							...
							for (var i = 0; i < noteMessageList.length; i++) {
								...
								if (receiverId && receiverId != factoryId && receiverInfo) {								
									...
									checkbox = '<input type="checkbox"/>';
									td = '<td><span class="w140">system information</span></td>';
									type = "（Received）";
								}
						...
						if (noteMessageList && noteMessageList.length > 0) {
							for (var i = 0; i < noteMessageList.length; i++) {
								...
								if (createTime) {
									...
									showTime = 'Date：' + createTime.substr(0, 4)
											+ '-' + createTime.substr(5, 2)
								...
								if (fileName && filePath) {
									...
									addMessage
											.find('.messageFile')
											.html(
													'Attachment:<a href="javaScript:void(0);" dataid='
															+ noteMessage.id
								...
								if (url) {
									if(factoryId != sendId || sendId == '0'){
										var detailurl = '<a href ="'
												+ url
												+ '" target="_blank" style="cursor: pointer;">Please click for details</a>'
								...
								if (i > 0) {
									$('#messageBody')
											.append(
													'<div class="original">------------ Original information ------------</div>')
								}
...
function replyMessage() {
	...
	if (!tempValue) {
		$('#detailIsNull').addClass('ff4').text('Please fill in the contents')
		return false;
	}
