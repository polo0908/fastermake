			    ...
			    if(data.country!=null && data.country!=''){
			    	...
			    	$("#country").find("option").each(function(){
			    		if($(this).text()==data.country){
			    			...
			    			//获取国家的电话区号
                            var code = getCodeByName(data.country);
                            if(code){
                            	$('.code').val('+'+code);
							}
			    		}
...
//			    	factoryGateHtml+='<div class="gs posiabso">（<span>0</span>/2）</div>';
				...
				for (var j = 0; j < qualificationList.length; j++) {
					...
					if(qualificationList[j].fileUrl!=null && qualificationList[j].fileUrl!=""){//已经上传过附件
						...
						qualificationHtml+='<td class="tdlast1"><span class="pull-left posirela"><em class="iconfont">&#xe72c;</em>Edit<input type="file" dataid="'+qualificationList[j].id+'" id="modifyQualificationPic" onchange="modifyQualification(this)" name="file" class="picfile"/></span>'
						qualificationHtml+='<span class="pull-right" onclick="delqualificationFile('+qualificationList[j].id+')"><em class="iconfont">&#xe616;</em>Delete</span></td></tr>'
...
function editFactoryInfo(){
	...
	$.ajax({
		...
		traditional : true,
		data : {
			factoryName:factoryName,aliWebsite:aliWebsite,tel:tel,mobile:mobile,country:country,factoryInfo:factoryInfo,
			province:state,detailsAddress:detailsAddress,establishmentYear:establishmentYear,
