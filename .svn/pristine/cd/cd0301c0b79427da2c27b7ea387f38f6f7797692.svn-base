			 ...
			 function(result){
			      if(result.state == 0){
			    	 ...
			    	 var isCollect = result.data.isCollect;
					  ...
					  $('meta[name="Description"]').attr('content',quoteInfo.quoteTitle + '询盘的详情信息');
                      // $('title').text(quoteInfo.quoteTitle + '询盘 快制造');
                      ...
                      if(supplierQuoteInfo && (supplierQuoteInfo.quoteStatus == 2 || supplierQuoteInfo.quoteStatus == 3 || supplierQuoteInfo.quoteStatus == 5) && quoteInfo.csgOrderId != null){
                          $('#title_ul').append('<li><a href="/report/reportList?csgOrderId='+quoteInfo.csgOrderId+'&supplierId='+supplierIdD+'">查看大货生产情况</a></li>');
                      }
			    	 ...
			    	 if(quoteInfo.orderStatus == 2){
                         ...
                         $('#order_status').text('<已授盘>');
			    	 }else if(quoteInfo.orderStatus == 6){
                         $('#order_status').text('<已生产>');
			   		 ...
			   		 $('#comparison_thead').find('tr').append('<th style="width:150px;">工艺</th>');
			    	 ...
			    	 if(quoteInfo.quoteLocation != 0){
			    		 $('#comparison_thead').find('tr').append('<th style="width:150px;">地域</th>');
			    	 ...
			    	 if(!(quoteInfo.equipmentKeywords == null || quoteInfo.equipmentKeywords == '' || quoteInfo.equipmentKeywords == undefined)){		    		 
			    		 $('#comparison_thead').find('tr').append('<th style="border-right: 1px solid #eee;width:105px;">设备要求</th>');
	    			 ...
	    			 if(drawingName && drawingPath){
                      ...
                      if(supplierQuoteInfo != null && supplierQuoteInfo != '' && supplierQuoteInfo != undefined){
                          ...
                          if(supplierQuoteInfo.quoteStatus == 1){
                              $('#quote_status').css({"color":"#ff973b"}).text('已报价');
                          }
                          if(supplierQuoteInfo.quoteStatus == 2){
                              $('#quote_status').css({"color":"#5FB1FE"}).text('已授盘给我');
                          }
                          if(supplierQuoteInfo.quoteStatus == 3){
                              $('#quote_status').css({"color":"#6dce59"}).text('已生产');
                          }
                          if(supplierQuoteInfo.quoteStatus == 4){
                              $('#quote_status').css({"color":"#999"}).text('已撤销');
                          }
                          if(supplierQuoteInfo.quoteStatus == 5){
                              $('#quote_status').css({"color":"#4c88ff"}).text('已完成');
                          }
                          if(supplierQuoteInfo.quoteStatus == 6){
                              $('#quote_status').css({"color":"#ccc"}).text('已过期');
                          }
                      }else{
                          $('#quote_status').text('未报价');
					  }
...
//			    	 }
				 ...
				 //                              (drawingPath == null ? '' : '<a href="###" class="fj1" onclick="download_drawing('+quoteInfo.orderId+')">图纸附件下载</a>')+
			                  		   ...
			                  		   '<tr class="add_notice_tr">'+
		                       			...
		                       			'<td colspan="7" class="change_colspan">'+(products[j].productRemark == null ? '无' : products[j].productRemark)+
		                       			'</td>'+      
				                  		/*'<td></td>'+
				                  		'<td></td>'+
				                  		'<td></td>'+
