			...
			if (result.state == 0) {
				...
				if (factoryList && factoryList.length > 0) {
					for (var i = 0; i < factoryList.length; i++) {
						...
						addHtml.find('.product').attr('href','/manufacturer-category/'+factoryInfo.factoryId+'/products')
						
						addHtml.find('.allproducts').attr('href','/manufacturer-category/'+factoryInfo.factoryId+'/products').children().text('查看全部产品'+(prListSize?prListSize:""))

						
						addHtml.find('.comments').attr('href','/manufacturer-category/'+factoryInfo.factoryId+'/comments')
