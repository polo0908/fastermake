			 ...
			 function(result){
			      if(result.state == 0){
					 ...
					 if(factoryInfo != null && factoryInfo != '' && factoryInfo != undefined){
					    	...
					    	$('#factory_advantage').find('p:eq(2)').find('span').text((factoryInfo.dominantMaterialSize == 0 ? 'Big' : (factoryInfo.dominantMaterialSize == 1 ? 'Middle' : (factoryInfo.dominantMaterialSize == 2 ? 'Small' : '-'))));
					    	$('#factory_advantage').find('p:eq(3)').find('span').text((factoryInfo.mainProcess == null ? '-' : factoryInfo.mainProcess));
					    	$('#factory_advantage').find('p:eq(4)').find('span').text((factoryInfo.siteSize == 0 ? 'Can meet the audit standards of the world\'s top 500 companies or the Tier 1 inspection standards of the auto parts industry' : (factoryInfo.siteSize == 1 ? 'Can pass the normal factory inspection' :(factoryInfo.siteSize == 2 ? 'Temporarily not allowed to check the factory' : '-'))));
					  }
...
//			    				 var split = fileName.split("&");
//			    				 fileName = split[0] + "." + gen;
			    				 file_div = '<div class="file-download">Attachment:<a style="text-decoration: underline;" onclick="download_file(\''+quoteMessages[i].id+'\')">'+quoteMessages[i].fileName+'</a></div>';	 
			    			 }		
			    	...
			    	for(var k=0;k<supplierQuoteProduct.length;k++){
			    		...
			    		if(k == 0){
				    		...
				    		for(var n=0;n<supplierQuoteProducts[k].length;n++){			    			
			                    ...
			                    var table = '<table class="table table-bordered pull-left table2">'+
				                    ...
				                    '<tr>'+
				                        ...
				                        '<th>Order</th>'+(supplierQuoteProducts[k][n].quoteUnitPrice2 == 0 ? '' : '<th>订量二</th>')+(supplierQuoteProducts[k][n].quoteUnitPrice3 == 0 ? '' : '<th>订量三</th>')+
				                    '</thead>'+
				                    '<tbody>'+
				                    '<tr class="f9">'+
				                        '<td>Qty</td>'+
				                    ...
				                    '</tr>'+
				                    '<tr >'+
				                        '<td>Total Unit Cost(CNY)</td>'+
				                        '<td>'+(supplierQuoteProducts[k][n].quoteProductPrice)+'</td>'+(supplierQuoteProducts[k][n].quoteUnitPrice2 == 0 ? '' : '<td>'+supplierQuoteProducts[k][n].quoteUnitPrice2+'</td>')+(supplierQuoteProducts[k][n].quoteUnitPrice3 == 0 ? '' : '<td>'+supplierQuoteProducts[k][n].quoteUnitPrice3+'</td>')+		       
				                    '</tr>'+
				                    '<tr class="f9">'+
				                        '<td>Total Mold Cost(CNY)</td>'+
				                    ...
				                    '<tr class="f9">'+
				                    '<td>Note</td>'+
				                    '<td>'+(supplierQuoteProduct[k].productRemark == null ? '' : supplierQuoteProduct[k].productRemark)+'</td>'+		       
				                    '</tr>'+
				                    '<tr class="f9">'+
				                    '<td>Total (CNY)</td>'+
				                    '<td>'+Number(supplierQuoteProducts[k][n].quoteProductPrice + supplierQuoteProducts[k][n].quoteMoldPrice1).toFixed(2)+'</td>'+(supplierQuoteProducts[k][n].quoteUnitPrice2 == 0 ? '' : '<td>'+Number(Number(qtyList[1]*supplierQuoteProducts[k][n].quoteUnitPrice2) + Number(supplierQuoteProducts[k][n].quoteMoldPrice2)).toFixed(2)+'</td>')+(supplierQuoteProducts[k][n].quoteUnitPrice3 == 0 ? '' : '<td>'+Number(Number(qtyList[2]*supplierQuoteProducts[k][n].quoteUnitPrice3) + Number(supplierQuoteProducts[k][n].quoteMoldPrice3)).toFixed(2)+'</td>')+		       
				                    '</tr>'+
				                    '</tbody>'+
				                '</table>';	
			                    tables +=table;
				    		}
				    		var currentQuote = '<p class="p1 clearfix"><em></em><span>Date：'+new Date(supplierQuoteProduct[k].createTime.replace(/-/g,"/").split(".")[0]).Format("yyyy-MM-dd hh:mm:ss")+' </span><em></em></p><div>'+tables+"</div>";
				    		...
				    		var historyQuote = '<p class="p1 clearfix"><i onclick="history(this)"></i><em></em><span>Date：'+new Date(supplierQuoteProduct[k].createTime.replace(/-/g,"/").split(".")[0]).Format("yyyy-MM-dd hh:mm:ss")+' </span><em></em></p><div>'+tables2+"</div>";
